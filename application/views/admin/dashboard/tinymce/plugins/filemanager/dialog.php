<?php
//start session for validate base path, prevent hacking
if(!isset($_SESSION)) {
	 session_start();
}
if($_SESSION['qd_basepath']!=1)
{
	echo 'You are not allowed on this area!';
	exit(0);
}
?>
...
if (isset($_GET['fldr']) && !empty($_GET['fldr'])) {
    $subdir = trim($_GET['fldr'],$qd_DIRECTORY_SEPARATOR) .$qd_DIRECTORY_SEPARATOR;
}
else
    $subdir = '';
...
$subfolder = '';
if(!empty($_SESSION["subfolder"]) && strpos($_SESSION["subfolder"],'..'.$qd_DIRECTORY_SEPARATOR)===FALSE
   && strpos($_SESSION["subfolder"],'.'.$qd_DIRECTORY_SEPARATOR)===FALSE && strpos($_SESSION["subfolder"],$qd_DIRECTORY_SEPARATOR)!==0
    && strpos($_SESSION["subfolder"],'.')===FALSE) $subfolder= $_SESSION['subfolder'];

if(trim($subfolder)==""){
    $cur_dir = $upload_dir . $subdir;
    $cur_path = $current_path . $subdir;
    $thumbs_path = 'thumbs'.$qd_DIRECTORY_SEPARATOR;
    $parent=$subdir;
}else{
    $cur_dir = $upload_dir . $subfolder . $qd_DIRECTORY_SEPARATOR . $subdir;
    $cur_path = $current_path . $subfolder .$qd_DIRECTORY_SEPARATOR. $subdir;
    $thumbs_path = 'thumbs'.$qd_DIRECTORY_SEPARATOR . $subfolder . $qd_DIRECTORY_SEPARATOR;
    $parent=$subfolder.$qd_DIRECTORY_SEPARATOR.$subdir;
}

$cycle=true;
while($cycle){
    if($parent==".".$qd_DIRECTORY_SEPARATOR) $parent="";    
    ...
    else $parent=dirname($parent).$qd_DIRECTORY_SEPARATOR;
}
...
<div class="uploader">
	...
	<div class="tabbable upload-tabbable"> <!-- Only required for left/right tabs -->
		...
		<form action="dialog.php" method="post" enctype="multipart/form-data" id="myAwesomeDropzone" class="dropzone">
		    ...
		    <input type="hidden" name="path_thumb" value="<?php /*echo $thumbs_path.$subdir;*/ echo $thumbs_path.$subdir; ?>"/>
		    <div class="fallback">
...
<!----- header div end ------->
	...
	<?php
	$bc=explode($qd_DIRECTORY_SEPARATOR,$subdir);
	$tmp_path='';
	if(!empty($bc))
	foreach($bc as $k=>$b){ 
		$tmp_path.=$b.$qd_DIRECTORY_SEPARATOR;
		if($k==count($bc)-2){
	?> <li class="active"><?php echo $b?></li><?php
		}elseif($b!=""){ ?>
		<li><a href="<?php echo $link.$tmp_path?>"><?php echo $b?></a></li><li><span class="divider"><?php echo $qd_DIRECTORY_SEPARATOR; ?></span></li>
	<?php }
	}
	?>
	...
	<div class="span12">	    
		...
		foreach ($files as $file) {
			...
			$class_ext = 3;			
			if($file=='..' && trim($subdir) != '' ){
			    $src = explode($qd_DIRECTORY_SEPARATOR,$subdir);
			    unset($src[count($src)-2]);
			    $src=implode($qd_DIRECTORY_SEPARATOR,$src);
			}
			elseif ($file!='..') {
			    $src = $subdir . $file.$qd_DIRECTORY_SEPARATOR;
			}
		    ...
		    foreach ($files as $nu=>$file) {
			    ...
			    $show_original=false;
			    $mini_src="";
			    $file_path=dirname( __FILE__ ). $qd_DIRECTORY_SEPARATOR.$current_path.$subfolder.$qd_DIRECTORY_SEPARATOR.$subdir.$file;
				if(in_array($file_ext, $ext_img)){
					$src = $base_url . $cur_dir . $file;
					$mini_src = $src_thumb = $thumbs_path.$subdir. $file;
					//add in thumbs folder if not exist 
					$thumb_path=str_replace($qd_DIRECTORY_SEPARATOR.$qd_DIRECTORY_SEPARATOR,$qd_DIRECTORY_SEPARATOR,dirname( __FILE__ ).$qd_DIRECTORY_SEPARATOR."thumbs".$qd_DIRECTORY_SEPARATOR.$subfolder.$qd_DIRECTORY_SEPARATOR.$subdir.$file);
					...
					if($img_width<122 && $img_height<91){
						$src_thumb=$current_path.$subfolder.$qd_DIRECTORY_SEPARATOR.$subdir.$file;
						$show_original=true;
					}
